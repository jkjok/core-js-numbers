...
function getAverage(value1, value2) {
  const s = (value1 + value2) / 2;
  return s !== Infinity ? (value1 + value2) / 2 : Number.MAX_VALUE;
}
...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const points = (x2 - x1) ** 2;
  const points2 = (y2 - y1) ** 2;
  return Math.sqrt(points + points2);
}
...
function toNumber(value, def) {
  if (value === null || Number.isNaN(Number(value))) return def;
  return value;
}
